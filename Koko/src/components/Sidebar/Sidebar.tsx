  ...
  return (
    <div className={`sidebar-container ${isCollapsed ? 'collapsed' : ''}`}>
      ...
      <nav className="sidebar-nav">
        ...
        <button 
          onClick={() => handleOptionClick('opera-style')}
          className={`sidebar-button ${selectedOption === 'opera-style' ? 'active' : ''}`}
          title={isCollapsed ? 'Opera Style' : ''}
        >
          <span className="sidebar-button-content">
            <OperaIcon />
            {!isCollapsed && <span className="sidebar-button-text">Opera Style</span>}
          </span>
        </button>
      </nav>
    </div>
